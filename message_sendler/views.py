...
from message_sendler.models import MailingSettings, Client, Message, MailingClient, MailingLog
from message_sendler.forms import MessageForm, MailingSettingsForm, ClientForm, MailingSettingsForManagerForm


class ContactsTemplateView(TemplateView):
    template_name = 'message_sendler/contacts.html'
    extra_context = {
        'title': 'Контакты'
    }
...
class HomeView(TemplateView):
    template_name = 'message_sendler/home.html'

    def get_context_data(self, **kwargs):
        context_data = super().get_context_data(**kwargs)
        context_data['count_mailing'] = MailingSettings.objects.all().count()
        context_data['count_mailing_active'] = MailingSettings.objects.filter(status=3).count()
...
class ClientListView(LoginRequiredMixin, ListView):
    model = Client
    template_name = 'message_sendler/client_list.html'
...
class ClientCreateView(LoginRequiredMixin, CreateView):
    ...
    template_name = 'message_sendler/client_form.html'
    success_url = reverse_lazy('message_sendler:client_list')
...
class ClientDeleteView(LoginRequiredMixin, DeleteView):
    model = Client
    template_name = 'message_sendler/client_confirm_delete.html'
    success_url = reverse_lazy('message_sendler:client_list')
...
class MailingSettingsListView(LoginRequiredMixin, ListView):
    ...
    template_name = 'message_sendler/mailinglist_list.html'
...
class MailingSettingsCreateView(LoginRequiredMixin, CreateView):
    ...
    template_name = 'message_sendler/mailinglist_form.html'
    success_url = reverse_lazy('message_sendler:mailing_list')
...
class MailingSettingsDeleteView(LoginRequiredMixin, DeleteView):
    model = MailingSettings
    template_name = 'message_sendler/mailinglist_confirm_delete.html'
    success_url = reverse_lazy('message_sendler:mailing_list')
    permission_required = 'message_sendler.change_mailingsettings'
...
class MessageListView(LoginRequiredMixin, ListView):
    model = Message
    template_name = 'message_sendler/message_list.html'
...
class MessageCreateView(LoginRequiredMixin, CreateView):
    ...
    template_name = 'message_sendler/message_form.html'
    success_url = reverse_lazy('message_sendler:message_list')
...
class MailingClintListView(PermissionRequiredMixin, ListView):
    model = MailingClient
    template_name = 'message_sendler/mailingclient_list.html'
    permission_required = 'message_sendler.view_mailingclient'
...
def toggle_client(request, pk, client_pk):
    ...
    return redirect(reverse('message_sendler:mailing_client', args=[pk]))
...
class MailingLogListView(ListView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset()
        if self.request.user.has_perm('message_sendler.view_mailinglog'):
            return queryset
        return queryset.filter(owner=self.request.user)
